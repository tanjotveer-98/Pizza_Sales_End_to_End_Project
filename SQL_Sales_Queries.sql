...
--6. Bottom 5 Pizza by Order count
...
--7. Lowest sales in each category.
with cte as(select
pizza_name, pizza_category, total_price,
row_number() over(partition by pizza_category order by total_price) as sales_rank
from pizza_sales)
select * from cte where sales_rank =1

--8.Highest Sales in each category.
with cte as(select
pizza_name, pizza_category, total_price,
row_number() over(partition by pizza_category order by total_price desc) as sales_rank
from pizza_sales)
select * from cte where sales_rank=1

--9.Pizza with lowest sales in each size
with cte as(select
pizza_name, pizza_size, total_price,
row_number() over(partition by pizza_size order by total_price) as sales_rank
from pizza_sales)
select * from cte where sales_rank=1

--10.Pizza with highest sales in each size
with cte as(select
pizza_name, pizza_size, total_price,
row_number() over(partition by pizza_size order by total_price desc) as sales_rank
from pizza_sales)
select distinct * from cte where sales_rank=1
